...
def view_user(user):
    ...
    print(
        f"{Fore.MAGENTA}Twitter\t\t{Fore.WHITE}{f'@{user.twitter}' if user.twitter else '-'}"
    )
...
def view_repo(repo):
    ...
    print(
        f"{Fore.MAGENTA}Created\t\t{Fore.WHITE}{dt.strftime(dt.fromisoformat(repo.created[:-1]), '%a %d %b,%Y')}"
    )
    print(
        f"{Fore.MAGENTA}Updated\t\t{Fore.WHITE}{dt.strftime(dt.fromisoformat(repo.updated[:-1]), '%a %d %b,%Y')}"
    )
    print("=" * 40)
